run_da_forecast <- function (states_init, pars_init = NULL, aux_states_init, obs, 
          obs_sd, model_sd, working_directory, met_file_names, inflow_file_names = NULL, 
          outflow_file_names = NULL, config, pars_config = NULL, states_config, 
          obs_config, management = NULL, da_method = "enkf", par_fit_method = "inflate") 
{
  ...
  x_init <- array(NA, dim = c(nmembers, nstates * ndepths_modeled + 
                                npars))
  for (m in 1:nmembers) {
    if (nstates > 1) {
      x_init[m, 1:(nstates * ndepths_modeled)] <- c(aperm(states_init[, 
                                                                      , m], perm = c(2, 1)))
    }
    else {
      x_init[m, 1:(nstates * ndepths_modeled)] <- states_init[1, 
                                                              , m]
    }
    if (!is.null(pars_init) & npars > 0) {
      x_init[m, (nstates * ndepths_modeled + 1):(nstates * 
                                                   ndepths_modeled + npars)] <- pars_init[, m]
    }
  }
  ...
  FLAREr:::check_enkf_inputs(states_init, pars_init, obs, 
                             psi, model_sd, config, pars_config, states_config, obs_config)
  ...
  if (length(config$output_settings$diagnostics_names) > 0) {
    diagnostics <- array(NA, dim = c(length(config$output_settings$diagnostics_names), 
                                     nsteps, ndepths_modeled, nmembers))
  }
  else {
    diagnostics <- NA
  }
  num_phytos <- length(which(stringr::str_detect(states_config$state_names, 
                                                 "PHY_") & !stringr::str_detect(states_config$state_names, 
                                                                                "_IP") & !stringr::str_detect(states_config$state_names, 
                                                                                                              "_IN")))
  full_time_char <- strftime(full_time, format = "%Y-%m-%d %H:%M", 
                             tz = "UTC")
  ...
  config$model_settings$ncore <- min(c(config$model_settings$ncore, 
  ...
  if (config$model_settings$ncore == 1) {
    ...
    FLAREr:::set_up_model(config, ens_working_directory = file.path(working_directory, 
                                                                    "1"), state_names = states_config$state_names, inflow_file_names = inflow_file_names, 
    ...
    lapply(1:nmembers, function(m) {
      ...
      FLAREr:::set_up_model(config, ens_working_directory = file.path(working_directory, 
                                                                      m), state_names = states_config$state_names, 
                            inflow_file_names = inflow_file_names, outflow_file_names = outflow_file_names)
    })
  }
  mixing_vars <- array(NA, dim = c(17, nsteps, nmembers))
  model_internal_depths <- array(NA, dim = c(nsteps, 500, 
                                             nmembers))
  ...
  config$metadata$model_description$version <- substr(glm_v, 
                                                      9, 16)
  for (i in start_step:nsteps) {
    curr_start <- strftime(full_time[i - 1], format = "%Y-%m-%d %H:%M", 
                           tz = "UTC")
    curr_stop <- strftime(full_time[i], format = "%Y-%m-%d %H:%M", 
                          tz = "UTC")
    message(paste0("Running time step ", i - 1, "/", (nsteps - 
                                                        1), " : ", curr_start, " - ", curr_stop, " [", Sys.time(), 
                   "]"))
    setwd(working_directory)
    met_index <- rep(1:length(met_file_names), times = nmembers)
    if (!is.null(ncol(inflow_file_names))) {
      inflow_outflow_index <- rep(1:nrow(inflow_file_names), 
                                  times = nmembers)
    }
    else {
    ...
    if (i == start_step) {
      ...
      parallel::clusterExport(cl, varlist = list("working_directory", 
                                                 "met_file_names", "met_index", "par_fit_method", 
                                                 "da_method", "nstates", "npars", "pars_config", 
                                                 "inflow_file_names", "inflow_outflow_index", 
                                                 "outflow_file_names", "curr_start", "curr_stop", 
                                                 "par_names", "par_nml", "num_phytos", "full_time", 
                                                 "management", "hist_days", "config", "states_config", 
                                                 "ndepths_modeled", "glm_output_vars", "num_wq_vars"), 
                              envir = environment())
    }
    parallel::clusterExport(cl, varlist = list("x", "i", 
                                               "mixing_vars", "model_internal_depths", "lake_depth", 
    ...
    if (i > 1) {
      out <- parallel::parLapply(cl, 1:nmembers, function(m) {
        ...
        if (npars > 0) {
          if (par_fit_method == "inflate" & da_method == 
              "enkf") {
            curr_pars_ens <- x[i - 1, m, (nstates + 
                                            1):(nstates + npars)]
          }
          else if (par_fit_method == "perturb" & da_method != 
                   "none") {
            if (i < (hist_days + 1)) {
              curr_pars_ens <- x[i - 1, m, (nstates + 
                                              1):(nstates + npars)] + rnorm(npars, 
                                                                            mean = rep(0, npars), sd = pars_config$perturb_par)
            }
            else {
              curr_pars_ens <- x[i - 1, m, (nstates + 
                                              1):(nstates + npars)]
        ...
        out <- FLAREr:::run_model(i, m, mixing_vars_start = mixing_vars[, 
                                                                        i - 1, m], curr_start, curr_stop, par_names, 
                                  curr_pars = curr_pars_ens, working_directory = file.path(working_directory, 
                                                                                           ens_dir_index), par_nml, num_phytos, glm_depths_start = model_internal_depths[i - 
                                                                                                                                                                           1, , m], lake_depth_start = lake_depth[i - 
                                                                                                                                                                                                                    1, m], x_start = x[i - 1, m, ], full_time, 
                                  wq_start = states_config$wq_start, wq_end = states_config$wq_end, 
                                  management = management, hist_days, modeled_depths = config$model_settings$modeled_depths, 
                                  ndepths_modeled, curr_met_file, inflow_file_name = inflow_file_name, 
                                  outflow_file_name = outflow_file_name, glm_output_vars = glm_output_vars, 
                                  diagnostics_names = config$output_settings$diagnostics_names, 
                                  npars, num_wq_vars, snow_ice_thickness_start = snow_ice_thickness[, 
                                                                                                    i - 1, m], avg_surf_temp_start = avg_surf_temp[i - 
                                                                                                                                                     1, m], salt_start = salt[i - 1, , m], nstates, 
                                  state_names = states_config$state_names, include_wq = config$include_wq)
      })
      for (m in 1:nmembers) {
        ...
        for (jj in 1:nrow(model_sd)) {
          ...
          for (kk in 2:ndepths_modeled) {
            w_new[kk] <- (alpha_v[jj] * w_new[kk - 1] + 
                            sqrt(1 - alpha_v[jj]^2) * w[kk])
            q_v[kk] <- w_new[kk] * model_sd[jj, kk]
          }
          x_corr[m, (((jj - 1) * ndepths_modeled) + 
                       1):(jj * ndepths_modeled)] <- x_star[m, 
                                                            (((jj - 1) * ndepths_modeled) + 1):(jj * 
                                                                                                  ndepths_modeled)] + q_v
        }
      }
      if (config$include_wq & config$da_setup$no_negative_states) {
        for (m in 1:nmembers) {
          index <- which(x_corr[m, ] < 0)
          x_corr[m, index[which(index <= states_config$wq_end[num_wq_vars + 
                                                                1] & index >= states_config$wq_start[2])]] <- 0
        }
      }
      if (npars > 0) {
        pars_corr <- curr_pars
        if (npars == 1) {
          pars_corr <- matrix(pars_corr, nrow = length(pars_corr), 
                              ncol = 1)
        }
        pars_star <- pars_corr
      }
    ...
    if (dim(obs)[1] > 1) {
      z_index <- which(!is.na(c(aperm(obs[, i, ], perm = c(2, 
                                                           1)))))
    }
    else {
      z_index <- which(!is.na(c(obs[1, i, ])))
    }
    if (length(z_index) == 0 | config$da_setup$da_method == 
        "none") {
      if (i > (hist_days + 1)) {
      ...
      else if (i <= (hist_days + 1) & config$uncertainty$use_obs_constraint) {
      ...
      if (npars > 0) {
        ...
        if (config$uncertainty$process == FALSE & i > 
            (hist_days + 1)) {
          x[i, , ] <- cbind(x_star, pars_star)
        }
        if (i == (hist_days + 1) & config$uncertainty$initial_condition == 
            FALSE) {
          for (m in 1:nmembers) {
      ...
      h <- matrix(0, nrow = length(obs_sd) * ndepths_modeled, 
                  ncol = nstates)
      index <- 0
        ...
        for (j in 1:ndepths_modeled) {
          ...
          if (!is.na(dplyr::first(states_config$states_to_obs[[k]]))) {
            for (jj in 1:length(states_config$states_to_obs[[k]])) {
              if (!is.na((obs[states_config$states_to_obs[[k]][jj], 
                              i, j]))) {
                states_to_obs_index <- states_config$states_to_obs[[k]][jj]
                index2 <- (states_to_obs_index - 1) * 
                  ndepths_modeled + j
      ...
      if (da_method == "enkf") {
        ...
        d_mat[which(z_index > length(config$model_settings$modeled_depths) & 
                      d_mat < 0)] <- 0
        ...
        if (npars > 0) {
          ...
          if (par_fit_method == "inflate") {
            for (m in 1:nmembers) {
              pars_corr[m, ] <- pars_config$inflat_pars * 
                (pars_corr[m, ] - par_mean) + par_mean
            }
            par_mean <- apply(pars_corr, 2, mean)
          }
        }
        ...
        if (!is.na(config$da_setup$localization_distance)) {
          p_t <- localization(p_t, nstates, modeled_depths = config$model_settings$modeled_depths, 
                              localization_distance = config$da_setup$localization_distance)
        }
        k_t <- p_t %*% t(h) %*% solve(h %*% p_t %*% 
                                        t(h) + psi_t, tol = 1e-17)
        if (npars > 0) {
          k_t_pars <- p_t_pars %*% t(h) %*% solve(h %*% 
                                                    p_t %*% t(h) + psi_t, tol = 1e-17)
        }
        ...
        for (m in 1:nmembers) {
          LL[m] <- sum(dnorm(zt, mean = obs_states[m, 
          ], sd = psi[z_index], log = TRUE))
        }
        sample <- sample.int(nmembers, replace = TRUE, 
                             prob = exp(LL))
        if (npars > 0) {
    ...
    if (i == (hist_days + 1)) {
      ...
      if (npars > 0) {
        if (config$uncertainty$parameter == FALSE) {
          par_means <- colMeans(x[i, , (nstates + 1):(nstates + 
                                                        npars)])
          for (m in 1:nmembers) {
    ...
    if (config$include_wq & config$da_setup$no_negative_states) {
      for (m in 1:nmembers) {
        index <- which(x[i, m, ] < 0)
        x[i, m, index[which(index <= states_config$wq_end[num_wq_vars + 
                                                            1] & index >= states_config$wq_start[2])]] <- 0
      }
    }
    if (npars > 0) {
      for (par in 1:npars) {
        ...
        high_index <- which(x[i, , nstates + par] > 
                              pars_config$par_upperbound[par])
    ...
    if (npars > 0) {
      for (par in 1:npars) {
        message(paste0(pars_config$par_names_save[par], 
                       ": mean ", round(mean(pars_corr[, par]), 4), 
                       " sd ", round(sd(pars_corr[, par]), 4)))
      }
    }
  }
  ...
  if (lubridate::day(full_time[hist_days + 1]) < 10) {
    file_name_F_day <- paste0("0", lubridate::day(full_time[hist_days + 
                                                              1]))
  }
  else {
    file_name_F_day <- lubridate::day(full_time[hist_days + 
                                                  1])
  }
  ...
  if (lubridate::month(full_time[hist_days + 1]) < 10) {
    file_name_F_month <- paste0("0", lubridate::month(full_time[hist_days + 
                                                                  1]))
  }
  else {
    file_name_F_month <- lubridate::month(full_time[hist_days + 
                                                      1])
  }
  ...
  curr_second <- round(lubridate::second(time_of_forecast), 
                       0)
  if (curr_day < 10) {
    curr_day <- paste0("0", curr_day)
  }
  if (curr_month < 10) {
    curr_month <- paste0("0", curr_month)
  }
  if (curr_hour < 10) {
    curr_hour <- paste0("0", curr_hour)
  }
  if (curr_minute < 10) {
    curr_minute <- paste0("0", curr_minute)
  }
  if (curr_second < 10) {
    curr_second <- paste0("0", curr_second)
  }
  forecast_iteration_id <- paste0(curr_year, curr_month, curr_day, 
                                  "T", curr_hour, curr_minute, curr_second)
  save_file_name <- paste0(config$run_config$sim_name, "_H_", 
                           (lubridate::year(full_time[1])), "_", file_name_H_month, 
                           "_", file_name_H_day, "_", (lubridate::year(full_time[hist_days + 
                                                                                   1])), "_", file_name_F_month, "_", file_name_F_day, 
                           "_F_", forecast_days, "_", forecast_iteration_id)
  for (m in 1:nmembers) {
    unlink(file.path(working_directory, m), recursive = TRUE)
  }
  return(list(full_time = full_time, forecast_start_datetime = forecast_start_datetime, 
              x = x, obs = obs, save_file_name = save_file_name, forecast_iteration_id = forecast_iteration_id, 
              forecast_project_id = config$run_config$sim_name, time_of_forecast = time_of_forecast, 
              mixing_vars = mixing_vars, snow_ice_thickness = snow_ice_thickness, 
              avg_surf_temp = avg_surf_temp, lake_depth = lake_depth, 
              salt = salt, model_internal_depths = model_internal_depths, 
              diagnostics = diagnostics, data_assimilation_flag = data_assimilation_flag, 
              forecast_flag = forecast_flag, da_qc_flag = da_qc_flag, 
              config = config, states_config = states_config, pars_config = pars_config, 
              obs_config = obs_config, met_file_names = met_file_names))
}
