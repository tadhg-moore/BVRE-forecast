plotting_obs_depths <- function (file_name, qaqc_data_directory, ncore = 1, plot_profile = TRUE, 
          obs_csv = TRUE) 
{
  ...
  source(file.path(lake_directory,"/R/combine_forecast_obs.R"))
  output <- combine_forecast_obs(file_name, 
                                 qaqc_location = qaqc_data_directory, extra_historical_days = 0, 
                                 ncore = ncore)
  obs <- output$obs
